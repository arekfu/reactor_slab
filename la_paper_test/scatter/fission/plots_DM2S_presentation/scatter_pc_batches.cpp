...
const int NPART = 1000; // Number of particles per batch
const int NBATCHES = 10000;
const int NBIN = 5;
...
void record_batch_scores() {
    ...
    sum_avg_coll_per_particle += (current_real_coll_tally / static_cast<double>(NPART));
    sum_avg_coll_per_particle_sqr += (current_real_coll_tally / static_cast<double>(NPART))*(current_real_coll_tally / static_cast<double>(NPART));
    sum_avg_all_coll_per_particle += (current_all_coll_tally / static_cast<double>(NPART));
    sum_avg_all_coll_per_particle_sqr += (current_all_coll_tally / static_cast<double>(NPART))*(current_all_coll_tally / static_cast<double>(NPART));
    sum_avg_escape += (current_escape_tally / static_cast<double>(NPART));
    sum_avg_escape_sqr += (current_escape_tally / static_cast<double>(NPART))*(current_escape_tally / static_cast<double>(NPART));

    for(int i = 0; i < NFOMBINS; i++) {
        sum_real_collision_density[i] += current_real_collision_density[i] / static_cast<double>(NPART);
        sum_real_collision_density_sqr[i] += (current_real_collision_density[i] / static_cast<double>(NPART))*(current_real_collision_density[i] / static_cast<double>(NPART));

        sum_all_collision_density[i] += current_all_collision_density[i] / static_cast<double>(NPART);
        sum_all_collision_density_sqr[i] += (current_all_collision_density[i] / static_cast<double>(NPART))*(current_all_collision_density[i] / static_cast<double>(NPART));
    }
}
...
int main() {
    ...
    for(int type = 1; type <= 6; type ++) {
        ...
        std::cout << "\n Carter Tracking\n";
        for(int b = 1; b <= NBATCHES; b++) {
            Carter_Transport(crs, 0.85, particle_bank);
            record_batch_scores();
            zero_current_batch_scores();
        }
        Output();
