...
const int ngens = 1015;
const int ninactive = 15;
const int ngroups = 2;
const int nmaterials = 2;
const double q = 0.3; // q = Preal
const double p = 0.7; // p = Esamp/Emaj
...
const double w_c = 0.25; // Cuttoff weight
const double w_s = 1.0; // Survival weight
...
double Initial_Eigenvalue(double Emaj[2], double** &Et, double** &Ea, double** &Ef,
    ...
    #pragma omp parallel
    {
        ...
        for(int i = 0; i < nparticles; i++) {
            ...
            double x = pos[i]; // Load position from bank
            bool alive = true; // Particle is alive
            int energy = 0; // Fast energy
            int material = 1; // Index for core

            while(alive) {
                double mu = 2.0*rand(rnd) - 1.0; // Isotropic scattering
                bool virtual_collision = true; // Collisions initially assumed to be virtual
                while(virtual_collision) {
                    ...
                    if(new_material != -1) { // Particle hasn't left simulation
                        if(rand(rnd) < (Et[new_material][energy]/Emaj[energy])) { // Virtual collision accepted
                            ...
                            x += d*mu; // Update position and material
                ...
                if (alive == true) {
                    ...
                    if(rand(rnd) < Ea[material][energy]/Et[material][energy]) { // Particle absorbed
                        #pragma omp atomic // Contribute to fission tally
...
void Write_Flux(double** &Flux_avg, double** &Flux_sqr_avg, int &nFbins) {
    std::ofstream output;
    output.open("nw_delta_tracking_flux.csv");
...
void Print_Data(int& g, double& H, double& k, double& kavg, double& std, high_resolution_clock::time_point& t0) {
    // Print table headers
    if(g == 1) {
    ...
    // print results depending on gen
    if(g > ninactive) {
        if(g > ninactive+1) {
...
void Eigenvalue(double Emaj[2], double** &Et, double** &Ea, double** &Ef, double*** &Es,
    ...
    double k_sum = 0.0; // Sum k
    double k_sqr_sum = 0.0; // Sum (k*k)

    double Esamp[2] = {p*Emaj[0], p*Emaj[1]};

    high_resolution_clock::time_point t0; // Starting timmer for FOM
    ...
    for(int g = 1; g <= ngens; g++) {
        ...
        #pragma omp parallel
        {
            ...
            for(int i = 0; i < nparticles; i++) {
                ...
                while(alive) {
                    double mu = 2.0*rand(rnd) - 1.0; // Isotropic scattering
                    double d = -std::log(rand(rnd))/Esamp[energy];
                    x += mu*d;
                    material = Get_Material(x);
                    if(material == -1) {alive = false;}
                    else {
                        if(rand(rnd) < Ea[material][energy]) { // Absorption
                            #pragma omp atomic
                            tally += w*v[material][energy]*Ef[material][energy]/Et[material][energy];

                        } 
                        /*
                        #pragma omp atomic // Record fission tally contribution from particle
                        tally += w*v[material][energy]*Ef[material][energy]/Et[material][energy];
			            ...
			            // Create new particles for next generation
                        ...
                        w = w*(1.0 - (Ea[material][energy]/Et[material][energy]));
...
int main() {
    std::cout << " Negative Weight Delta Tracking Two Group Reflected Slab\n\n";
