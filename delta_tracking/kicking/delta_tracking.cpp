...
void Write_Flux(double** &Flux_avg, double** &Flux_sqr_avg, int &nFbins) {
    std::ofstream output;
    output.open("delta_tracking_flux.csv");
...
void Eigenvalue(double Emaj[2], double** &Et, double** &Ea, double** &Ef, double*** &Es,
    ...
    for(int g = 1; g <= ngens; g++) {
        ...
        #pragma omp parallel
        {
			...
			// Creat new particles for next generation
...
int main() {
    std::cout << " Delta Tracking (Kicking) Two Group Reflected Slab\n\n";
