...
double Initial_Eigenvalue(double** &Et, double** &Ea, double** &Ef,
    ...
    #pragma omp parallel
    {
        ...
        for(int i = 0; i < nparticles; i++) {
            ...
            while(alive) {
                ...
                bool ray_trace = true;
                while(ray_trace) {
                    double d = -std::log(rand(rnd))/Et[material][energy]; // Distance to collision in material
                    double d_surf = Distance_to_Surface(x, mu); // Distance to nearest surface in particle direction

                    if (d < d_surf) { // No surfaces crossed, move particle
                        x += d*mu;
                        ray_trace = false;
                    }
                    else { // Surface crossed
                        double d_surf_orig = d_surf;
                        while(x+(d_surf*mu) == x+(d_surf_orig*mu)) { // Increase distance untill surface is barely crossed
                            d_surf = std::nextafter(d_surf, INFINITY);
                        }
                        x += d_surf*mu; // Move particle to other side of surface
                        material = Get_Material(x); // Get new material of cell
                        if(material == -1) { // Particle left reactor, kill it
                            ray_trace = false;
                            alive = false;
                        }
                    }
...
void Write_Flux(double** &Flux_avg, double** &Flux_sqr_avg, int &nFbins) {
    std::ofstream output;
    output.open("ray_trace_flux.csv");
...
void Eigenvalue(double** &Et, double** &Ea, double** &Ef, double*** &Es, double*** &Es1, 
                double** &v, double* &pos, double* &pos_future, double &k) {
    ...
    for(int g = 1; g <= ngens; g++) {
        ...
        if(g > ninactive) {
            ...
            if(g > ninactive+1) {
                ...
                std::cout << kavg << "  " << std::fixed << std::setprecision(5) << std << "  " << std::fixed << std::setprecision(5) << (int)FOM << "\n";
            }
            else {
                std::cout << " " << std::setfill(' ') << std::setw(4) << g << "  " << std::fixed << std::setprecision(5) << k;
...
int main() {
    std::cout << " Ray Tracing Two Group Reflected Slab\n\n";
    ...
    double k = Initial_Eigenvalue(Et, Ea, Ef, Es, Es1, v, x);
    std::cout << " Initial multiplication factor: " << k << "\n\n";    

    // Perform iterations for eigen value
    Eigenvalue(Et, Ea, Ef, Es, Es1, v, x, x_F, k);
