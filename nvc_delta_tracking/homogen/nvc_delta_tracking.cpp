...
const double Surfaces[2] = {0.0, 100.0};
...
void Generate_Source(double* &x, double* &x_F) {
    ...
    for(int i = 0; i < nparticles; i++) {
        x[i] = (Surfaces[1]-Surfaces[0])*rand(rnd);
    }
}
            ...
            while(alive) {
                ...
                while(virtual_collision) {
                    ...
                    if(new_material != -1) { // Particle hasn't left simulation
                        if(rand(rnd) < (Et[energy]/Emaj[energy])) { // Virtual collision accepted
                ...
                if (alive == true) {
                    ...
                    if(rand(rnd) < Ea[energy]/Et[energy]) { // Particle absorbed
                        #pragma omp atomic
                        tally += v[energy]*Ef[energy]/Ea[energy];
                        alive = false;
                    }
                    else { // Particle scattered, to what group?
                        double scatter_total = Es[energy][0] + Es[energy][1];
                        if(rand(rnd) < Es[energy][0]/scatter_total) { energy = 0; }
                        else { energy = 1; }
                    }
                }
            }
...
void Eigenvalue(double Emaj[2], double* &Et, double* &Ea, double* &Ef, double** &Es,
    ...
    int nHbins = std::round((Surfaces[1] - Surfaces[0])/Hdx);
    double* Entropy;
    ...
    // Flux bin allocation
    int nFbins = std::round((Surfaces[1] - Surfaces[0])/Fdx);
    double** Flux;
    double** Flux_sum;
    double** Flux_sqr_sum;
    Flux = new double*[2];
            ...
            for(int i = 0; i < nparticles; i++) {
                ...
                while(alive) {
                    ...
                    if(alive == true) {
                        ...
                        tally += w*(1.0 - (Et[energy]/Emaj[energy]))*v[energy]*Ef[energy]/Et[energy];
                        ...
                        if(g > ninactive) {
                            ...
                            Flux[energy][bin] += w*(1.0 - (Et[energy]/Emaj[energy]))/(Et[energy]);
                        }

                        //int new_particles = floor((w/k)*v[material][energy]*Ef[material][energy]/Et[material][energy] + rand(rnd));
                        int new_particles = floor((w*(1.0 - (Et[energy]/Emaj[energy]))/k)*v[energy]*Ef[energy]/Et[energy] + rand(rnd));
                        ...
                        w = w*(Et[energy]/Emaj[energy])*(1.0 - (Ea[energy]/Et[energy]));
